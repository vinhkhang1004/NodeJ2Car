...
const Register = () => {
    ...
    return (
        <div className="animate-fade-in w-full">
            ...
            <p className="text-slate-500 mb-8 text-sm leading-relaxed">Đăng ký để truy cập kho phụ tùng cao cấp và quản lý đơn hàng của bạn.</p>
            ...
            <div className="relative mb-8 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                <span className="bg-white px-3 relative z-10">Hoặc đăng ký bằng Email</span>
            ...
            <form onSubmit={submitHandler} className="space-y-6">
                <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Tên Đầy Đủ</label>
                    <input 
                        type="text" 
                ...
                <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Mật khẩu</label>
                ...
                <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Xác nhận mật khẩu</label>
                    <div className="relative">
                        <input 
                            type={showPassword ? "text" : "password"} 
                            placeholder="••••••••"
                ...
                <button type="submit" disabled={loading} className="mt-4 w-full bg-[#f97316] text-white font-bold py-4 rounded-md hover:bg-[#ea580c] transition-colors flex justify-center items-center shadow-lg shadow-orange-500/20 text-sm">
                    {loading ? <Loader2 className="animate-spin" size={20} /> : (
                        <>ĐĂNG KÝ HỆ THỐNG <ArrowRight className="ml-2 h-4 w-4" /></>
                    )}
                </button>
            </form>
            ...
            <div className="mt-8 text-center text-xs font-medium text-slate-600">
                ...
                <Link to={redirect ? `/login?redirect=${redirect}` : '/login'} className="font-bold text-blue-900 hover:text-blue-700 ml-1">
                    Đăng nhập ngay
                </Link>
            </div>

            <div className="mt-12 pt-6 border-t border-slate-100 flex justify-between items-center text-[10px] uppercase tracking-wider font-bold text-slate-400">
